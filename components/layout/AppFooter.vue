<template>
  <footer class="mx-auto p-6 pt-0 max-w-6xl">
    <div
      class="flex items-center justify-between pt-6 border-t border-gray-800"
    >
      <p class="text-gray-500 text-sm">
        {{ footer.copyright }} &copy; {{ currentYear }}
      </p>
      <ul class="flex items-center">
        <li v-if="footer.githubUrl" class="ml-2">
          <a :href="footer.githubUrl" title="Find me on GitHub">
            <svg
              class="h-6 w-6 text-gray-500 duration-300 ease-in-out transition fill-current focus:text-white hover:text-white"
        ...
        <li v-if="footer.twitterUrl" class="ml-2">
          <a :href="footer.twitterUrl" title="Find me on Twitter">
            <svg
              class="h-6 w-6 text-gray-500 duration-300 ease-in-out transition fill-current focus:text-white hover:text-white"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20"
  ...
  computed: {
    footer() {
      ...
      return {
        ...
        githubUrl: global.github_url?.cached_url,
        twitterUrl: global.twitter_url?.cached_url
      }
    }
  }
}
