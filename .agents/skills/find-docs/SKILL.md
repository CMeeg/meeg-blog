...
description: >-
  ...
  For Microsoft/Azure products it falls back to official Microsoft Learn documentation.
...
5. If no good matches exist and the query appears to be about a Microsoft product (Azure, .NET, C#, PowerShell, TypeScript, Office, SharePoint, Dynamics, Visual Studio, Win32, MSDN, etc.), proceed to the **Microsoft Learn Fallback** workflow below instead of giving up
6. For ambiguous queries, request clarification before proceeding with a best-guess match
...
2. If the query appears to be about a Microsoft product, fall back to the **Microsoft Learn Fallback** workflow below
3. Otherwise, suggest they authenticate for higher limits: `ctx7 login`
4. If they cannot or choose not to authenticate, answer from training knowledge and clearly note it may be outdated

Do not silently fall back to training data — always tell the user why Context7 was not used.

## Microsoft Learn Fallback

When `ctx7 library` returns no good matches and the query appears to be about a Microsoft product (Azure, .NET, C#, PowerShell, TypeScript, Office, SharePoint, Dynamics, Visual Studio, Win32, MSDN, etc.), use the Microsoft Learn MCP tools to search official documentation.

### Step 1: Search Documentation

Use `microsoft-learn_microsoft_docs_search` with the query to find relevant official documentation pages:

```
microsoft-learn_microsoft_docs_search query="Azure Functions bindings"
```

This returns up to 10 content chunks (max 500 tokens each) with article titles, URLs, and excerpts.

### Step 2: Fetch Full Pages

For high-value results, use `microsoft-learn_microsoft_docs_fetch` with the full URL to retrieve the complete page content:

```
microsoft-learn_microsoft_docs_fetch url="https://learn.microsoft.com/en-us/azure/azure-functions/functions-bindings"
```

### Step 3: Code Samples (if applicable)

If the user needs code examples in a specific language, use `microsoft-learn_microsoft_code_sample_search`:

```
microsoft-learn_microsoft_code_sample_search query="Azure Functions bindings" language="csharp"
```

Supported languages: csharp, javascript, typescript, python, powershell, azurecli, al, sql, java, kusto, cpp, go, rust, ruby, php.
