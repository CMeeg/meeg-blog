...
- **Singleton**: an ES module with exported bindings is already a singleton. Use the class form only when you need lazy init or to swap the instance in tests. *However*, also consider build/bundler output and runtime target, for example frameworks may create separate bundles for server/middleware/API routes, and serverless/edge runtimes that run code in isolated and short-lifetime processes will not share the same module scope.
...
- **Observer**: `EventTarget`, `EventEmitter`, or an RxJS `Subject` are battle-tested implementations — hand-roll only for learning or tight control.
